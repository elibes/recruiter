...
class AuthenticationService {
  ...
  /**
   * This function hashes user passwords, it is used for hashing the password before inserting into the db.
   *
  ...
  static async hashPassword(plainTextPassword: string): Promise<string> {
    try {
      return await bcrypt.hash(plainTextPassword, AuthenticationService.SALT_ROUNDS);
    } catch (error) {
